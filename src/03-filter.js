...
function getSongsBySaib(songs) {
  ...
  // })
  // return arr;
let saib = songs.filter((song) => {return song.artist === "Saib";})
return saib
}
...
function getSongsOverThreeMinutes(songs) {
  let time = songs.filter((song) => {return song.runtimeInSeconds > 180})
  return time;
}
...
function getTitleTracks(songs) {
  let same = songs.filter((song) => {return song.title === song.album})
  return same;
}

...
let under = songs.every((song) => {song.runtimeInSeconds > 120}) 
  return under 
}
...
function anySongIsOverFourMinutes(songs) {
  let overFour = songs.some((song) => {song.runtimeInSeconds > 240})
  return overFour
}
...
function anySongIsByPeanut(songs) {
  let songByPea = songs.some((song) => song.artist === "Peanut");
  return songByPea
}
